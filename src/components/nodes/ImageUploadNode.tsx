...
import type { ImageUploadNodeData } from '@/lib/types';
...
export default function ImageUploadNode({ id, data, selected }: ImageUploadNodeProps) {
    ...
    const handleFileSelect = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                updateNodeData(id, { imageUrl: result.url, fileName: file.name, output: result.url });
            }
    ...
    return (
        <div className={`relative group w-[260px] rounded-xl bg-[#121212] border transition-all duration-200 ${statusClass} ${selected ? 'border-[#D4FF3F]' : 'hover:border-[#3a3a3a]'}`}>
            ...
            <div className="flex items-center gap-2 px-3 py-2.5 border-b border-[#1a1a1a]">
                <div className="w-5 h-5 rounded-md bg-gradient-to-br from-orange-500 to-red-500 flex items-center justify-center shadow-lg shadow-orange-900/20">
                    <ImageIcon className="w-3 h-3 text-white" />
                </div>
                <span className="text-[12px] font-medium text-[#e5e5e5] tracking-wide">{data.label || 'Image'}</span>
                {status === 'running' && <Loader2 className="w-3 h-3 animate-spin text-[#D4FF3F] ml-auto" />}
            </div>

            {/* Content */}
            <div className="p-3">
                <input ref={fileInputRef} type="file" accept="image/*" onChange={handleFileSelect} className="hidden" disabled={isExecuting || isUploading} />

                {data.imageUrl ? (
                    <div className="relative group">
                        <img src={data.imageUrl} alt="" className="w-full h-24 object-cover rounded" />
                    ...
                    <button
                        ...
                        className="w-full h-20 border border-dashed border-[#333] rounded flex flex-col items-center justify-center gap-1 hover:border-[#D4FF3F]/40 hover:bg-[#D4FF3F]/5 transition-colors"
